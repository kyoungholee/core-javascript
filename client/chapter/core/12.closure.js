...
// 여기서는 number() 함수가 클로져가 된다.

/**
 * 1. 왜? number 함수가 inner 함수 자체를 받아서 count에 넣었기 
 * 2. 상위 함수의 생명주기가 끝나도 (실행컨텍스트) 하위 함수에서 상위 함수의 값을 참조 할 수 있는거
 * 이건 하위 함수의 클로져다 
 * 즉 클로져는 전역을 오염시키지 않고 아래 함수 혹은 부무의 참조 값을 볼 수 있냐 이거다 !! 
 * 
 */
function number() {
  let count = 0;
